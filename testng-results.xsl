    ...
    <xsl:template match="/testng-results">
        ...
        <xsl:result-document href="{testng:absolutePath('overview.html')}" format="xhtml">
            <html xmlns="http://www.w3.org/1999/xhtml">
                <xsl:call-template name="htmlHead"/>
                <body>
                    <h2>MSB Test suites overview</h2>
                    <table width="100%">
                        <tr>
	   ...
	   <tr style="background-color: #eaf0f7;">
            <td width="100%">
               <div class="suiteStatusFail"></div>Test Case 
            </td>
            <td xmlns="" style="background-color: padding: 3px 3px 3px 0;"
                align="center">
               <div style="width: 50px;">Fail</div>
            </td>
            <td xmlns="" style="background-color: padding: 3px 3px 3px 0;"
                align="center">
        ...
        <xsl:for-each select="$methodList">
            ...
            <tr id="{concat($methodId, '_row')}" class="{testng:testMethodStatus(.)}">
                <xsl:if test="testng:isFilterSelected(@status) != 'true'">
                    <!--<xsl:attribute name="style" select="'display: none;'"/>   onclick="toggleDetailsVisibility('{$detailsId}')  onclick="toggleDetailsVisibility('{$exceptionDetailsId}')"  -->
                </xsl:if>
                ...
                <td nowrap="true">
					...
					<!--This code is added by raghu for generating screen shots in xslt report for failed TC's-->
                        <a  target="_blank" href="e:\\test.png"> 
                            <xsl:value-of select="concat( exception/message,'')"/>
                        </a>
						<img src="e:\\test.png" alt="Smiley face" height="42" width="42"/>
